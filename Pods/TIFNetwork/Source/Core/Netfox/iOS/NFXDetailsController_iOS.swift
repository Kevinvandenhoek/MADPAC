...
final class NFXDetailsController_iOS: NFXDetailsController, MFMailComposeViewControllerDelegate {
    ...
    let urlToFind = "https://appstubs.triple-it.nl/"
    ...
    @objc func actionButtonPressed(_ sender: UIBarButtonItem) {
        let parameters: Parameters = [
            "app": Bundle.main.infoDictionary?[kCFBundleNameKey as String] ?? "Onbekend",
            "bundleid": Bundle.main.bundleIdentifier ?? "",
            "url": selectedModel.requestURL ?? "Onbekend",
            "json": self.selectedModel.getResponseBody()
        ]
        
        // All three of these calls are equivalent
        Alamofire.request("https://appstubs.triple-it.nl/receiver.php", method: .post, parameters: parameters)
        
        let alert = UIAlertController(title: "JSON sended", message: "You can find the JSON in the browser with URL: \n\n \(urlToFind)", preferredStyle: UIAlertControllerStyle.alert)
