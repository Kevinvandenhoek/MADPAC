...
class NFXGenericController: NFXViewController
{
    ...
    func formatNFXString(_ string: String) -> NSAttributedString
    {
        ...
        for match in matchesBodyHeaders {
            tempMutableString.addAttribute(NSAttributedStringKey.font, value: NFXFont.NFXFontBold(14), range: match.range)
            tempMutableString.addAttribute(NSAttributedStringKey.foregroundColor, value: NFXColor.black, range: match.range)
        }
